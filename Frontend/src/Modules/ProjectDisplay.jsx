...
const projects = [
  {
    ...
    techStack: ["NEXT JS 14", "APP ROUTER", "TAILWIND CSS", "SHADCN UI", "SUPABASE", "FIRECRAWL", "RESEND", "CRON JOBS"],
    image: offery,
...
const ProjectDisplayWithVSCode = () => {
  ...
  return (
    <section className="max-w-screen-2xl px-6 sm:px-24 mx-auto relative pb-8 sm:pb-16 bg-[#111827] overflow-x-hidden">
      ...
      {visibleProjects.map((project, index) => (
        <ProjectCardWithVSCode key={project.title} {...project} />
      ))}
      ...
      >
        <button
          ...
          className="group relative px-8 py-3 font-semibold text-sm sm:text-base text-white bg-gradient-to-r from-[#175845] to-[#42c6a1] rounded-full shadow-lg shadow-emerald-500/20 hover:shadow-emerald-500/40 transition-all duration-300 hover:scale-105 transform-gpu"
        >
...
export default ProjectDisplayWithVSCode
