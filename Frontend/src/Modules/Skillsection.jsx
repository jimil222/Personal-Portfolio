...
export default function SkillsSection() {
  ...
  const skills = [
    ...
    },
    {
      ...
      technologies: ["Git", "GitHub", "Vs code", "Postman", "Canva"],
      ...
      <div className="container mx-auto px-4 relative">
        ...
        >
          {skills.map((skill, index) => (
            <motion.div
              key={index}
              className={`relative bg-gray-800 rounded-lg p-6 shadow-lg transition-all duration-300 w-full sm:w-[calc(50%-1rem)] lg:w-[calc(33.333%-1rem)] max-w-sm cursor-pointer ${hoveredSkill === index ? 'shadow-2xl scale-102' : 'hover:shadow-xl hover:scale-101'}`}
              variants={itemVariants}
              onHoverStart={() => setHoveredSkill(index)}
              onHoverEnd={() => setHoveredSkill(null)}
            >
              <div className={`absolute inset-0 bg-gradient-to-br ${skill.color} opacity-20 rounded-lg py-3`} />
